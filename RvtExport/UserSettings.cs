    ...
    class UserSettings
    {
        ...
        public static bool JsonIndented
        {
            get
            {
                ...
                if (!File.Exists(path))
                {
                    File.WriteAllText(path, _JsonIndent + "=" + Boolean.TrueString);
                    //Util.ErrorMsg(string.Format("Created a new user settings file at '{0}'.", path));
                }
                ...
                string s = s1.Substring(i + _JsonIndent.Length);
